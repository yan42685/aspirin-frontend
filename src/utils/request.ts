...
export function putRequest<T>(url: string, params?: any): Promise<T> {
  return httpClient.put(url, qs.stringify(params));
}

export function deleteRequest<T>(url: string, params?: any): Promise<T> {
  return httpClient.delete(url, { params: qs.stringify(params) });
}

export function patchRequest<T>(url: string, params?: any): Promise<T> {
