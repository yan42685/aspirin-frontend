...
function getCourseDescription(course: TeacherTeachDTO): string {
  return (
    course.courseDetailId + "@" + course.courseName + "\n" + course.courseNumber
  );
}

// function populateCourseList(list: TeacherTeachDTO[]) {
//   // 清空courseList
//   for (let i = 0; i < 7; i++) {
//     const oneDayCourses = [];
//     for (let j = 0; j < 12; j++) {
...
export function renderCourseTable(list: TeacherTeachDTO[]) {
  // populateCourseList(list);

  timeTable = new Timetables({
    el: "#teacherTeachTable",
    timetables: list,
    week: week,
...
export function updateCourseTable(list: TeacherTeachDTO[]) {
  // populateCourseList(list);
